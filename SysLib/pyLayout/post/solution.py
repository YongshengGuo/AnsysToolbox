...
class Solution(object):
    ...
    def __init__(self,name = None,setupName = None, sweepName = None,variation = None,layout=None):
        
        if name and ":" in name:
            self.name = name
            self.setupName,self.sweepName = name.split(":",1)
        elif setupName and sweepName:
            self.setupName = setupName
            self.sweepName = sweepName
            self.name = "%s:%s"%(setupName,sweepName)
        else:
            pass
    ...
    @property
    def Name(self):
        return self.name or "%s_%s"%(self.setupName,self.sweepName)
    ...
    def exportNetworkData(self,path = None):
        solutionName = "%s:%s"%(self.setupName,self.sweepName)
        ext = ".s%sp"%len(self.layout.Ports)
        if not path:
#             path = self.layout.projectDir + "\%s"%solutionName
            path = os.path.join(self.layout.projectDir,"_".join([self.layout.projectName,self.layout.designName,self.setupName,self.sweepName])+ext)
            
        variation_array=self.oModule.ListVariations(solutionName)
        self.oDesign.ExportNetworkData("", [solutionName], 3, path, ["ALL"], True, 50, "S", -1, 0, 15)
